...
def model_train(
    ...
    for epoch in range(train_epochs):
        ...
        pbar = tqdm(train_iter, desc=f"Epoch {epoch + 1:03d}/{train_epochs} [Train]", leave=False, dynamic_ncols=True)
...
def model_eval(
    ...
    if eval_method == "common":
        ...
        with torch.no_grad():
            ...
            pbar = tqdm(test_iter, desc="Evaluating (Holmes)", leave=False, dynamic_ncols=True)
            for _, cur_data in enumerate(pbar):
                cur_X, cur_y = cur_data[0].to(device), cur_data[1].to(device)
